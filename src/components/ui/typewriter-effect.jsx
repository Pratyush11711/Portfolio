    ...
    return (
      <motion.div ref={scope} className="inline">
        {wordsArray.map((word, idx) => (
          <motion.span
            key={`word-${idx}`}
            className={cn("dark:text-white text-black opacity-0", words[idx].className)}
          >
            {word}
            {idx < wordsArray.length - 1 && <span>&nbsp;</span>} {/* Adding space between words */}
          </motion.span>
        ))}
      </motion.div>
    );
      ...
      <div>
        {words.map((word, idx) => (
          <span key={`word-${idx}`} className={cn("dark:text-white text-black", word.className)}>
            {word.text}
            {idx < words.length - 1 && <span>&nbsp;</span>} {/* Adding space between words */}
          </span>
        ))}
      </div>
    );
